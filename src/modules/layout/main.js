...
function setSign(units) {
  if (units === "metric") sign = "°C";
  else sign = "°F";
}

function setWindSpeed(units) {
  if (units === "metric") windSpeed = "m/s";
  else windSpeed = "m/h";
}
...
const icons = {
  "01d": `/${BASE_URL}/icons/sun.png`,
  "01n": `/${BASE_URL}/icons/crescent-moon.png`,
  "02d": `/${BASE_URL}/icons/sun-cloudy.png`,
  "02n": `/${BASE_URL}/icons/moon-cloudy.png`,
  "03d": `/${BASE_URL}/icons/clouds.png`,
  "03n": `/${BASE_URL}/icons/clouds.png`,
  "04d": `/${BASE_URL}/icons/clouds.png`,
  "04n": `/${BASE_URL}/icons/clouds.png`,
  "09d": `/${BASE_URL}/icons/heavy-rain.png`,
  "09n": `/${BASE_URL}/icons/heavy-rain.png`,
  "10d": `/${BASE_URL}/icons/rain.png`,
  "10n": `/${BASE_URL}/icons/rain.png`,
  "11d": `/${BASE_URL}/icons/thunderstorm.png`,
  "11n": `/${BASE_URL}/icons/thunderstorm.png`,
  "13d": `/${BASE_URL}/icons/snow.png`,
  "13n": `/${BASE_URL}/icons/snow.png`,
  "50d": `/${BASE_URL}/icons/fog.png`,
  "50n": `/${BASE_URL}/icons/fog.png`,
};
...
function createSummaryLeft(obj) {
  ...
  const description =
    obj.description.charAt(0).toUpperCase() + obj.description.slice(1);
...
function createSummaryCenter(obj) {
  ...
  center.appendChild(
    createCenterElem(`/${BASE_URL}/icons/location.png`, location)
  );
  center.appendChild(
    createCenterElem(`/${BASE_URL}/icons/clock.png`, setTime(obj.time))
  );
...
function createSummaryRight(obj) {
  ...
  right.appendChild(
    createRightElem(
      `/${BASE_URL}/icons/temperature.png`,
      "Feels Like",
      obj.feels_like + sign
    )
  );
  ...
  right.appendChild(
    createRightElem(
      `/${BASE_URL}/icons/rain.png`,
      "Precipitations",
      obj.precipitations + "%"
    )
  );
  right.appendChild(
    createRightElem(`/${BASE_URL}/icons/wind.png`, "Wind", obj.wind + windSpeed)
  );
...
function createAdditionalInfo(obj) {
  ...
  additional.appendChild(
    createAdditionalElem("CLOUDINESS", obj.cloudiness + "%")
  );
  additional.appendChild(createAdditionalElem("UV Index", obj.uvi));
  additional.appendChild(
    createAdditionalElem("PRESSURE", obj.pressure + "hPa")
  );
...
function createDetails(chosen) {
  ...
  details.appendChild(
    createAdditionalElem("DAY", floor(chosen.feels_like.day) + sign)
  );
  details.appendChild(
    createAdditionalElem("NIGHT", floor(chosen.feels_like.night) + sign)
  );
  details.appendChild(createAdditionalElem("HUMIDITY", chosen.humidity + "%"));
  details.appendChild(
    createAdditionalElem("PRECIPITATIONS", (chosen.pop * 100).toFixed(0) + "%")
  );
  details.appendChild(
    createAdditionalElem("WIND", chosen.wind_speed + windSpeed)
  );
  ...
  details.appendChild(
    createAdditionalElem("PRESSURE", chosen.pressure + "hPa")
  );
...
function populateDetails(e, chosen) {
  const stringAux =
    e.target.parentNode.getAttribute("id") || e.target.getAttribute("id");
  ...
  details.appendChild(
    createAdditionalElem("DAY", floor(chosen[currId].feels_like.day) + sign)
  );
  details.appendChild(
    createAdditionalElem("NIGHT", floor(chosen[currId].feels_like.night) + sign)
  );
  details.appendChild(
    createAdditionalElem("HUMIDITY", chosen[currId].humidity + "%")
  );
  details.appendChild(
    createAdditionalElem(
      "PRECIPITATIONS",
      (chosen[currId].pop * 100).toFixed(0) + "%"
    )
  );
  details.appendChild(
    createAdditionalElem(
      "WIND",
      chosen[currId].wind_speed.toFixed(1) + windSpeed
    )
  );
  details.appendChild(
    createAdditionalElem("SUNRISE", getHour(chosen[currId].sunrise))
  );
  details.appendChild(
    createAdditionalElem("SUNSET", getHour(chosen[currId].sunset))
  );
  details.appendChild(
    createAdditionalElem("CLOUDINESS", chosen[currId].clouds + "%")
  );
  details.appendChild(createAdditionalElem("UV Index", chosen[currId].uvi));
  details.appendChild(
    createAdditionalElem("PRESSURE", chosen[currId].pressure + "hPa")
  );
}
...
function displayDaily() {
  ...
  if (window.innerWidth > 650) daily.style.display = "grid";
...
function displayHourly() {
  ...
  if (dailies) dailies.classList.remove("daily_active");

  details.style.display = "none";

  daily.style.display = "none";
  if (window.innerWidth > 500) hourly.style.display = "grid";
...
function displayWithBullet(e, id) {
  ...
  if (window.innerWidth > 500) show.style.display = "grid";
  else show.style.display = "flex";
}

function displayWithArrows(value) {
  if (currentId + value > 3) currentId = 0;
  if (currentId + value < 1) currentId = 4;
  ...
  if (window.innerWidth > 500) hourlies[currentId - 1].style.display = "grid";
...
function dailyGridElem(obj) {
  ...
  wind.src = `/${BASE_URL}/icons/wind.png`;
  ...
  rain.src = `/${BASE_URL}/icons/rain.png`;
